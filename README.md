...
### 🔧 树莓派 4B USB 引导问题 ([#67](https://github.com/bigbugcc/OpenWrts/issues/67))

**问题概述：** 近期固件版本中，树莓派 4B 无法通过 USB 正常启动，部分版本甚至 SD 卡也无法启动。

**详细说明：**
- 新版固件的 `cmdline.txt` 中 `root` 参数格式已从 `root=/dev/mmcblk0p2` 变更为 `root=PARTUUID=xxx-02`，导致以往通过修改为 `root=/dev/sda2` 实现 USB 引导的方式失效。
- 2025.10.17 之后的固件版本存在无法启动的问题（USB 和 SD 卡均受影响）。
- 部分版本启动后 WiFi 不可用（无 SSID 广播），内置 BCM43455 无线未正常工作。
- 自行编译的用户需要手动添加对应型号的 USB 网卡驱动。

**临时解决方案：**
- 使用 2025.10.17 或更早的固件版本。
- WiFi 问题已尝试修复，请关注最新的 [Release](https://github.com/bigbugcc/OpenWrts/releases)。

**相关用户：** @Chickenbi24 @xiyig @bxiao0719 @likunbase-a11y
